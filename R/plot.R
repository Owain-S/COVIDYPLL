...
plot_age_adjusted_ypll <- function(dt, byvar, year_rle = NA,
  ...
  g_out <- ggplot(data = all_ypll) +
    ...
    ylab(paste0("Age-Adjusted Years of Potential Life Lost Rate\n(Per 100,000 Population)")) +
    ggtitle(title) +
...
#' @import data.table
#' @export
plot_ratio_ypll_to_covid <- function(dt, byvar, year_rle = NA,
                                     usa_ypll_ls = NULL,
                                     panel_letter = NULL,
                                     axis.text.y.size = 8) {
  if (is.null(usa_ypll_ls)) {
  ...
  ratio_ypll_death[, `:=` (prop_covid = covid_19_deaths / sum(covid_19_deaths),
                                 prop_ypll = tot_ypll / sum(tot_ypll))]

  ratio_ypll_death[, ratio := prop_ypll / prop_covid]
  ...
  g_out <- ggplot(data = ratio_ypll_death) +
    ...
    ggtitle(title) +
    xlab("Ratio of YPLL to COVID-19 deaths") +
