...
data {
  ...
  int<lower=1> Jmi[Nmi]; // index of missing values
  int<lower=1> Jmi_gp[Nmi]; // gp index among missing values
  int<lower=1> n_miss_sd; // number of the SDs by the grouping of missing values
...
parameters {
  // Missing data
  vector<lower=0.6, upper=9.4>[Nmi] Ymi;
  vector<lower=0>[n_miss_sd] v_sd_miss; // vector of SDs for the grouping of missing values
  // Multilevel model
...
model {
  ...
  Yl[Jmi] = Ymi;
  for (n in 1:Nmi) {
    tmp_miss_gp = Jmi_gp[n];
    vec_sd_miss[n] = v_sd_miss[tmp_miss_gp];
  }
  ...
  target += exponential_lpdf(v_sd_miss | 0.14); // mean is around 7
  for (n in 1:Nmi) {
    target += normal_lpdf(Ymi[n] | 1, vec_sd_miss[n]);
  }
