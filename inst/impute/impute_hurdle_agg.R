...
begin_time <- Sys.time()
fit_hurdle <- stan(
  file = "inst/impute/stan/impute_hurdle_agg.stan",  # Stan program
...
saveRDS(fit_hurdle, "inst/impute/results/fit_hurdle_agg.RDS")


fit_hurdle <- readRDS("inst/impute/results/fit_hurdle_agg.RDS")

pars <- c("bQ", "shape", "b_hu")
sum_estimates <- round(summary(fit_hurdle, pars = pars, probs = c(0.025, 0.975))$summary, 3)
saveRDS(sum_estimates, "inst/impute/results/sum_estimates_hurdle_agg.RDS")

traceplot(fit_hurdle, pars = pars)
ggsave("inst/impute/results/traceplot of fixed effects_hurdle_agg.png", device = "png",
       height = 12, width = 14)
...
saveRDS(list(ix_miss = ix_miss,
             ymis_draws = ymis_draws), "inst/impute/bayes_impute_agg.RDS")
...
ggsave("inst/impute/results/impute distribution_hurdle_agg.tiff", device = "tiff", height = 12, width = 12)
...
g2 <- ggplot(data = covid19deaths_sum[y_cate != "0"]) +
  ...
  theme(plot.title = element_text(size = 18, hjust = 0.5),
        ...
        axis.title.y = element_text(size = 14),
        legend.position = "bottom")
ggsave("inst/impute/results/impute distribution_age_hurdle_agg.png",
...
for (x in rucc_ls) {
  ...
  ggsave(paste0("inst/impute/results/impute distribution_hurdle_agg (", x, ").png"),
         plot = g2, device = "png", height = 10, width = 18)
}
...
ggplot(data = covid19deaths_sum[y_cate != "0"]) +
  ...
  theme(plot.title = element_text(size = 20, hjust = 0.5),
        ...
        axis.title.y = element_text(size = 14),
        legend.position = "bottom")
ggsave(paste0("inst/impute/results/distribution simulated and data_hurdle_agg.png"),
...
ggsave(paste0("inst/impute/results/distribution simulated and data prop zeros_hurdle_agg.png"),
...
for (i in unique(covid19d_cty$urban_rural_code)) {
  ...
  ggplot(data = covid19deaths_sum[urban_rural_code == i & y_cate != "0"]) +
    ...
    theme(plot.title = element_text(size = 20, hjust = 0.5),
          ...
          axis.title.y = element_text(size = 14),
          legend.position = "bottom")
  save_file <- paste0("inst/impute/results/distribution simulated and data hurdle_agg (", i, ").png")
