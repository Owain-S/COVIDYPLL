...
ggsave(paste0("inst/impute/results/impute distribution_hurdle_agg", i,".tiff"), device = "tiff", height = 12, width = 12)
...
for (i in unique(covid19d_cty$urban_rural_code)) {
  ...
  ggplot(data = covid19deaths_sum[urban_rural_code == i & y_cate != "0"]) +
    ...
    theme(plot.title = element_text(size = 20, hjust = 0.5),
          ...
          axis.title.y = element_text(size = 14),
          legend.position = "bottom")
  save_file <- paste0("inst/impute/results/distribution simulated and data hurdle_agg", i,"(", i, ").png")
